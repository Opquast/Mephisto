...
const {extend, parseQueryString} = require("utils");
...
// * onLoadWait(): Called 1 sec after onLoad
...
function RequestProcessor(request, response, options) {
    ...
    this.onLoadWait = this.onLoadWait.bind(this);
...
RequestProcessor.prototype = {
    ...
    onLoad: function(evt) {
        try {
            ...
            setTimeout(this.onLoadWait, 1000);
    ...
    onLoadWait: function() {
        try {
            ...
            for (var i=0; i<modifiers.length; i++) {
                let _modifier = this.evalJSURI(modifiers[i]);
                if (typeof(_modifier) == 'object') {
                    modifier_result.push(_modifier);
                }
            }
            
            if (typeof(this.options.onLoadWait) !== "undefined") {
                this.options.onLoadWait.call(this, modifier_result);
            }
    ...
    evalJSURI: function(uri) {
        if (uri.search(/^http(s?):\/\//) == -1) {
            try {
                let content = self.data.load('modifiers/' + uri);
            ...
            try {
                ...
                req.send(null);
                if (req.status == 200) {
                    return this.evalJS(req.responseText);
                }
            } catch (e) {
                console.error('Unable to open modifier ' + uri);
                console.error(e);
