...
const {cleanMemory, cleanSTS, doCC, extend, parseQueryString, CookieParser} = require("./utils");

const ash = Cc["@mozilla.org/appshell/appShellService;1"].getService(Ci.nsIAppShellService);
const wm = Cc["@mozilla.org/appshell/window-mediator;1"].getService(Ci.nsIWindowMediator);

const force_refresh = Ci.nsIWebNavigation.LOAD_FLAGS_BYPASS_CACHE;

// Time after we stop page load
const load_timeout = preferences.get('extensions.mephisto.loadTimeout') || 20;

// Time to run modifiers
const close_timeout = preferences.get('extensions.mephisto.closeTimeout') || 80;
...
RequestProcessor.prototype = {
    closeTab: function(remove_event) {
        ...
        // Remove close events added by shity pages
        if (this.tab !== null) {
            extend(this.tabBrowser.contentDocument.defaultView.wrappedJSObject, {
    ...
    process: function() {
        ...
        this.timeout = setTimeout(this.onTimeout, load_timeout * 1000);
    ...
    onRequestModify: function(request) {
        ...
        request.setRequestHeader('Vary', 'Accept-Encoding, Accept-Language, Via', false);
    ...
    onRequestLogStart: function(request, listener) {
        try {
            // Remove stored STS information if any
            cleanSTS(request.URI);
    ...
    _fullClose: function() {
        ...
        var t = setTimeout(function() {
            _this.tabBrowser.stop();
            _this.closeTab();
        }, close_timeout * 1000);

        this.onBeforeClose();
        if (this.tabBrowser.stop != undefined) {
            clearTimeout(t);
            this.tabBrowser.stop();
            doCC(this.tabBrowser.contentWindow);
            this.closeTab();
        }
    },

    onBeforeClose: function() {
        console.info("CLOSING TAB - " + this.request.GET.get("url"));
        try {
    ...
    onClose: function(evt) {
        try {
            console.info("TAB CLOSED - " + this.request.GET.get("url"));
    ...
    takeScreenshot: function(width, height) {
        ...
        let w = this.tabBrowser.contentDocument.body.offsetWidth;
        let h = height * w/width;

        let canvas = ash.hiddenDOMWindow.document.createElementNS("http://www.w3.org/1999/xhtml", "canvas");
        canvas.width = width;
        ...
        let result = canvas.toDataURL().split(',', 2)[1];
        delete(ctx, canvas);
    ...
    callModifiers: function(sidecar) {
        // Get default modifiers first
        let modifiers = preferences.get('extensions.mephisto.defaultModifiers') || "";
        modifiers = modifiers.split(/\s*,\s*/);

        // Then get modifiers from request args
        this.request.GET.getlist('modifier').forEach(function(e) {
            if (modifiers.indexOf(e) == -1) {
                modifiers.push(e);
            }
        });
    ...
    evalJS: function(code, sidecar) {
        if (this.sandbox == null) {
            ...
            // Screenshot helper
            this.sandbox.takeScreenshot = this.takeScreenshot.bind(this);
        }
        ...
        if (uri.search(/^(http|https):\/\//) == 0) {
            ...
            try {
                ...
                if (uri.search(/^file:\/\//) == 0) {
                    fn = uri.replace(/^file:\/\//, '');
                    if (!file.isFile(fn)) {
                        throw "File " + fn + " does not exist.";
                    }
                    data = file.read(fn);
                } else {
                    try {
                        data = self.data.load("modifiers/" + uri)
                    } catch(e) {
                        throw "Resource " + self.data.url("modifiers/" + uri) + " does not exist.";
                    }
                }
                return this.evalJS(data, sidecar);
            } catch(e) {
                console.error('Unable to open modifier ' + uri);
                console.error(e);
            }
