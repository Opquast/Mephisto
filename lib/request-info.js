...
exports.request_info = function(request, listener) {
    let out = {
        ...
        status_text: unescape(encodeURIComponent(request.responseStatusText)),
        date: null,
        modified: null,
        expires: null,
        content_type: null,
    ...
    if (out.content_type !== null && out.content_type.search(/^image\//) !== -1) {
        ...
        if (out.size > 0) {
            [out.image_info.width, out.image_info.height, out.image_info.animated] = get_image_info(out.uri, out.content_type, listener);
        }
    }
    return out;
};
...
var get_image_info = function(uri, content_type, listener) {
    ...
    */
    ...
    try {
        ...
        console.debug("Failed to load image information - " + uri);
        return [0,0, false];
    }
};
