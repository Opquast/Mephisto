        ...
        onClose: function(evt) {
            try {
                ...
                function sendResults() {
                    _self.response.setHeader('Content-Type', 'application/json; charset=UTF-8', false);
                    _self.response.write(JSON.stringify(_self.result));
                }
                
                function fetchResults() {
                    var _done = true;
                    
                    for (var i in _self.result) {
                        if (_self.result[i].state) {
                            if(_self.result[i].state() == "resolved") {
                                _done = true;
                                
                                _self.result[i].done(function() {
                                    _self.result[i] = JSON.parse(unescape(encodeURIComponent(JSON.stringify(arguments[0]))));
                                });
                            } else {
                                _done = false;
                            }
