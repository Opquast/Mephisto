...
exports.loaders = {
    ...
    '/dump': {
        ...
        onBeforeClose: function() {
            ...
            // Events
            const esl = Cc["@mozilla.org/eventlistenerservice;1"].getService(Components.interfaces.nsIEventListenerService);
            let tw = this.tabBrowser.contentDocument.createTreeWalker(
            	this.tabBrowser.contentDocument,
			...
			while (tw.nextNode()) {
				esl.getListenerInfoFor(tw.currentNode,{}).forEach(function(value, key, array) {
					if(value.toSource()){
						// xpath
						let xpath = "";
						let _item = tw.currentNode;
						
						for(; _item && _item.nodeType == 1; _item = _item.parentNode) {
							//
							let idx = 1;
							let xname = _item.localName;
							...
							if(_item.hasAttribute("id")) {
								...
								for(let sib = _item.previousSibling; sib; sib = sib.previousSibling) {
									if(sib.nodeType == 1 && sib.localName == xname) {
										idx++;
									}
								}
    ...
    '/screenshot': {
        ...
        onBeforeClose: function() {
            ...
            this.result = canvas.mozGetAsFile("screenshot", "image/png");
            delete(ctx, canvas);
        },
        onClose: function(evt) {
            var result = this.result.getAsBinary();
            if (this.request.hasHeader('accept-encoding') && this.request.getHeader('accept-encoding').search("base64") != -1) {
                var result = this.tabBrowser.contentWindow.btoa(result);
