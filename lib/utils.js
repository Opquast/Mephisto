...
const sts = Cc["@mozilla.org/stsservice;1"].getService(Ci.nsIStrictTransportSecurityService);
...
function _isPlainObject(obj) {
    ...
    if ( !obj || _type(obj) !== "object" || obj.nodeType || (obj != null && obj == obj.window) ) {
        return false;
    }
...
exports.memoryReport = memoryReport;

const cleanMemory = function(aWindow) {
    Cu.forceGC();

    mgr.minimizeMemoryUsage(function() {
        let rss = memoryReport()["resident"];
        console.info("MINIMIZED MEMORY (" + rss + "MB)");
    });
};
exports.cleanMemory = cleanMemory;

const doCC = function(aWindow) {
...
const cleanSTS = function(aURI) {
    try {
        if (sts.isStsHost(aURI.host)) {
            sts.removeStsState(aURI);
            console.debug("STS state removed for " + aURI.host);
        }
    } catch(e) {
        console.exception(e);
    }
};
exports.cleanSTS = cleanSTS;
